...
<!-- Responsible for: the authoritative, human-readable database schema description. The schema's source of truth is backend/prisma/schema.prisma (Prisma); applied SQL lives in backend/prisma/migrations/ (generated by `prisma migrate dev`, applied automatically by the backend container's `prisma migrate deploy`). -->
