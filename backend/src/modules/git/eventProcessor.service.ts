...
export async function processPullRequestEvent(payload: GitHubPullRequestPayload): Promise<void> {
    ...
    const {action, pull_request} = payload; //desrtcurisation
    if(action != 'opened' && action != 'reopened')
        return;
    const cardId = extractCardID(pull_request.head.ref) || extractCardID(pull_request.title);
    if(cardId){
        console.log(`[Git Service] PR #${cardId} open. Status 'PR pending'`);
    await transitionCardStatus(cardId.toString(), 'PR pending')
    }
}
...
export async function processMergeEvent(payload: GitHubPullRequestPayload): Promise<void> {
    ...
    if(action == 'closed' && pull_request.merged == true){
        const cardId = extractCardID(pull_request.head.ref) || extractCardID(pull_request.title);
        if(cardId){
            console.log(`[Git Service] PR #${cardId} merge in main. Status 'Done'`);
            await transitionCardStatus(cardId.toString(), 'Done')
        }
    }
}
...
export async function transitionCardStatus(cardId: string, newStatus: string): Promise<void> {
    ...
    //????
    console.log(`[Kanban Action] Carde ${cardId} transition in new status.`);
}
