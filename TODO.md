...
- `infra/nginx/default.conf.template` *(nginx image template; ${HTTP_PORT}/${HTTPS_PORT} from .env)*
- `backend/prisma/migrations/` *(generated by `prisma migrate dev`; applied on container start via `prisma migrate deploy`)*
